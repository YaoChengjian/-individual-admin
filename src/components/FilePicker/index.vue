...
<template>
  <YModal
    ...
    v-model="visible"
    :loading="loading"
    class="y-file-picker-modal y-modal-show-header-border y-modal-show-footer-border"
    @open="handleOpen"
    @close="handleClose"
  >
    <YSplitPanel
...
<script lang="ts" setup>
  ...
  import { useMessage, useMessageBox, findTree } from 'y-element-ultra';
  import type { ElEmptyProps } from 'y-element-ultra/es/y-app/el';
  import type {
    YModalProps,
    YDropdownInstance,
    YDropdownProps,
  ...
  const props = withDefaults(
    defineProps<{
      ...
      /** 分组下拉菜单属性 */
      groupDropdownProps?: YDropdownProps;
      /** 添加分组弹窗参数 */
      groupAddModalProps?: Omit<YModalProps, 'modelValue'>;
      /** 重命名弹窗参数 */
      renameModalProps?: Omit<YModalProps, 'modelValue'>;
      /** 移动分组弹窗参数 */
      moveModalProps?: Omit<YModalProps, 'modelValue'>;
      /** 文件列表自定义属性 */
      fileListProps?: YFileListProps;
      /** 已选列表自定义属性 */
      selectionListProps?: YUploadListProps;
      /** 空组件属性 */
      emptyProps?: ElEmptyProps;
      /** 分页组件属性 */
      paginationProps?: YPaginationProps;
      /** 统一设置层级 */
      baseIndex?: number;
      /** 自定义文案 */
  ...
  /** 右键菜单组件 */
  const ctxMenuDropdownRef = ref<YDropdownInstance>();
