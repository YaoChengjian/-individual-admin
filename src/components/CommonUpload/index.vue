...
<script lang="ts" setup>
  ...
  import type {
    ElProgressProps,
    ElImageProps
  } from 'y-element-ultra/es/y-app/el';
  import type { YImageViewerProps } from 'y-element-ultra/es/y-app/plus';
  import type {
    UploadItem,
    EditUploadResult,
    ListType,
    ...
    BeforePreview,
    UploadLocale
  } from 'y-element-ultra/es/y-upload-list/types';
  ...
  const props = withDefaults(
    defineProps<{
      ...
      /** 自定义图片预览属性 */
      previewProps?: YImageViewerProps;
      /** 是否开启底部预览和修改的操作按钮 */
      tools?: boolean;
      /** 列表显示样式 */
      listType?: ListType;
  ...
  const checkFile = (file?: File) => {
    ...
    if (props.accept === 'image/*') {
      if (!file.type.startsWith('image')) {
        YMessage.error({ message: lang.value.imageError, plain: true });
        return;
      }
      ...
      if (
        ...
        YMessage.error({ message: lang.value.excelError, plain: true });
        return;
      }
    }
