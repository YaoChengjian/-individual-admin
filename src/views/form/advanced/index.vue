<template>
  <y-page plain :multi-card="false">
    <y-card style="border-radius: 0">
      <y-text type="heading" size="lg">复杂表单</y-text>
    ...
    <y-page>
      ...
      >
        ...
        <y-card header="选择成员">
          ...
          >
            <y-table style="min-width: 580px; table-layout: fixed">
              <colgroup>
                <col width="60px" />
                <col />
                <col />
              ...
              <tbody style="--y-table-padding: 6px 8px">
                ...
                <tr v-if="!form.users || !form.users.length">
                  <td colspan="5" style="text-align: center">
                    <y-text style="padding: 4px 0" type="secondary">
                      暂无数据
                    </y-text>
                  </td>
                </tr>
    ...
    <!-- 底部工具栏 -->
    <y-bottom-bar teleported>
      <y-text v-if="validMsg" type="danger" :icon="CloseCircleOutlined">
        <span>{{ validMsg }}</span>
      </y-text>
      <template #extra>
...
<script lang="ts" setup>
  ...
  const submit = () => {
    formRef.value?.validate?.((valid, obj) => {
      ...
      setTimeout(() => {
        loading.value = false;
        YMessage.success({ message: '提交成功', plain: true });
        resetFields();
      }, 1000);
    });
  };
...
<style lang="scss" scoped>
  .user-table-wrap {
    ...
    &.is-ping-left :deep(.user-table-index) {
      backdrop-filter: var(--y-table-fixed-backdrop-filter);
      ...
      &::before {
        ...
        bottom: -1px;
        right: -10px;
        box-shadow: var(--y-table-fixed-left-shadow);
    ...
    &.is-ping-right :deep(.user-table-action) {
      ...
      &::before {
        ...
        bottom: -1px;
        left: -10px;
        box-shadow: var(--y-table-fixed-right-shadow);
        transition: box-shadow 0.2s;
        pointer-events: none;
      }
    }
