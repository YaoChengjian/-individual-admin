<template>
  <div>
    ...
    >
      ...
      <template #status="{ row }">
        <y-text v-if="row.status === 0" type="warning">未读</y-text>
        <y-text v-else-if="row.status === 1" type="info">已读</y-text>
      </template>
      <template #action="{ row }">
...
<script lang="ts" setup>
  ...
  /** 回复 */
  const reply = (row: UserMessage) => {
    messageIns.info(`暂未接入回复发送接口：${row.title}`);
  };
  ...
  const readBatch = () => {
    ...
    $http(
      updateUserMessageStatus(
        'letter',
        selections.value.map((item) => item.id)
      ),
      (res) => {
        handleSuccess(res.message);
      },
