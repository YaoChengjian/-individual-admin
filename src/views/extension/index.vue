<template>
  ...
  >
    <div class="demo-extension-nav">
      <y-card :flex-table="true" :body-style="{ padding: 0 }">
        <el-scrollbar>
          <y-menus
            ref="menuRef"
            mode="compact"
...
<script lang="ts" setup>
  ...
  const active = ref<string>('table/index.vue');

  const menuRef = ref<YMenusInstance>(null);

  const wrapRef = ref<YPageInstance>(null);

  const bodyComponent = shallowRef<Component>();
