<template>
  <y-card header="更多示例">
    ...
    <y-printer v-model="printing1" target="_iframe">
      <div style="font-size: 22px; margin-bottom: 24px; line-height: 1.2">
        表格打印示例：
      </div>
      <y-table size="large" :border="true" style="page-break-after: always">
        <colgroup>
          <col width="60px" />
          <col width="200px" />
          <col width="120px" />
        ...
        <tbody>
          <tr v-for="(row, index) in projects" :key="row.id">
            ...
            <td>{{ row.endDate }}</td>
            <td>
              <y-text v-if="row.status === 0" type="success">进行中</y-text>
              <y-text v-else-if="row.status === 1" type="danger">
                已延期
              </y-text>
              <y-text v-else-if="row.status === 2" type="warning">
                未开始
              </y-text>
              <y-text v-else-if="row.status === 3" type="info">
                已结束
              </y-text>
            </td>
          </tr>
      ...
      <y-table
        ...
        :style="{
          ...
          '--y-table-bg': '#fff',
          '--y-table-color': '#000',
          '--y-table-border-color': '#000',
          '--y-table-th-color': '#000',
          '--y-table-th-font-weight': 'bold',
          '--y-table-th-bg': 'transparent',
          '--y-table-tr-bg': 'transparent',
          '--y-table-tr-hover-bg': 'transparent',
          '--y-table-radius': 0,
          '--y-table-sm-radius': 0,
          '--y-table-lg-radius': 0,
        ...
        <y-table>
          ...
          <tbody>
            <tr v-for="(row, index) in projects" :key="row.id">
              ...
              <td>{{ row.endDate }}</td>
              <td>
                <y-text v-if="row.status === 0" type="success">
                  进行中
                </y-text>
                <y-text v-else-if="row.status === 1" type="danger">
                  已延期
    ...
    <y-printer v-model="printing2" target="_iframe">
      <div style="font-size: 22px; margin-bottom: 24px; line-height: 1.2">
        图片打印示例：
      </div>
      <img
    ...
    <y-printer v-model="printing3" target="_iframe">
      <div style="font-size: 22px; margin-bottom: 24px; line-height: 1.2">
        二维码打印示例：
      </div>
      <div
      ...
      >
        <y-qr-code-svg :value="partyA" :size="180" />
        <div style="font-size: 14px; line-height: 1.2">{{ partyA }}</div>
      </div>
    </y-printer>
    <y-printer v-model="printing4" target="_iframe">
      <div style="font-size: 22px; margin-bottom: 24px; line-height: 1.2">
        水印组件：
      </div>
      <y-watermark
    ...
    <y-printer v-model="printing5" target="_iframe">
      <div style="font-size: 22px; margin-bottom: 24px; line-height: 1.2">
        echarts图表打印示例：
      </div>
      <v-chart
...
<script lang="ts" setup>
  ...
  import { ChartTheme } from 'y-element-ultra';
