<template>
  <y-page>
        ...
        <template #item="{ item }">
          <div style="display: flex; padding: 18px 12px; height: 87px">
            ...
            <div style="flex: 1; overflow: hidden; padding-left: 12px">
              <y-text size="md">{{ item?.title }}</y-text>
              <y-ellipsis size="sm" type="secondary" :max-line="3">
                {{ item?.content }}
              </y-ellipsis>
            </div>
          </div>
    ...
    <y-card :collapsable="true" :body-style="{ padding: 0 }" header="单选模式">
      <div style="padding: 20px">
        <y-check-card v-model="checked2" :items="items" :row="{ gutter: 12 }">
          <template #item="{ item }">
            <div style="display: flex; padding: 18px 12px; height: 87px">
              <el-avatar :size="46" shape="square" :src="item?.avatar" />
              <div style="flex: 1; overflow: hidden; padding-left: 12px">
...
<style lang="scss">
  ...
  .y-check-card-group > .y-check-card:nth-child(2) .demo-card-pay-img {
    &::before {
      width: 32px;
  ...
  .y-check-card-group > .y-check-card:last-child .demo-card-pay-img {
    &::before {
      width: 38px;
