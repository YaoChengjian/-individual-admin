<template>
  <y-card header="Popover、PopConfirm、Dropdown">
    <div>
      <el-space wrap style="margin-bottom: 20px">
        ...
        >
          <template #reference>
            <el-button class="y-btn-icon">Popover</el-button>
          </template>
        ...
        >
          <template #reference>
            <el-button class="y-btn-icon">PopConfirm</el-button>
          </template>
        </y-popconfirm>
        <y-popconfirm
          :width="258"
        ...
        >
          <template #reference>
            <el-button class="y-btn-icon">PopConfirm 自定义按钮</el-button>
          </template>
    ...
    <div>
      <el-space wrap>
        ...
        <y-dropdown
          ...
          trigger="click"
          :split-button="true"
          :split-button-props="{ class: 'y-btn-icon' }"
          @command="handleCommand"
          @click="handleClick"
        >
          <span>按钮组</span>
...
<script lang="ts" setup>
  ...
  const handleCommand = (command: string) => {
    YMessage.info({ message: `点击了: ${command}`, plain: true });
  };

  const handleClick = () => {
    YMessage.info({ message: '点击了按钮组', plain: true });
  };

  const handleConfirm = () => {
    YMessage.info({ message: '点击了确定', plain: true });
  };

  const handleAgree = () => {
    YMessage.info({ message: '点击了满意', plain: true });
  };

  const handleComplain = (cancel: any) => {
    cancel && cancel();
    YMessage.info({ message: '点击了投诉', plain: true });
  };
</script>
