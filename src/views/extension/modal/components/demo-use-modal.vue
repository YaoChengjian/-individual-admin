<template>
  <y-card>
    ...
    <div style="margin-top: 12px">
      <el-button class="y-btn-icon" @click="openDialog">打开弹窗</el-button>
      <el-button class="y-btn-icon" @click="openDialog2">打开抽屉</el-button>
      <el-button class="y-btn-icon" @click="openDialog3">
        打开无遮罩层弹窗
      </el-button>
    </div>
    <div style="margin-top: 16px">
      <el-button class="y-btn-icon" @click="openDialog4">
        传递数据及监听保存成功事件
      </el-button>
    </div>
    <div style="margin-top: 16px">
      <el-button class="y-btn-icon" @click="openDialog5">
        页面关闭后仍然保持弹窗打开
      </el-button>
    </div>
    <div style="margin-top: 16px">
      <el-button class="y-btn-icon" @click="openDialog6">
        自定义弹窗的顶栏、底栏
      </el-button>
    </div>
  </y-card>
...
<script lang="ts" setup>
  import { useModal, YMessage } from 'y-element-ultra';
  ...
  const openDialog4 = () => {
    ...
    openModal({
      ...
      componentProps: {
        data: row,
        onDone: () => {
          YMessage.success({ message: '监听到保存成功事件', plain: true });
        }
      }
    });
  };
