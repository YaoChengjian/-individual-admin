<template>
  <y-card header="调用文件选择器">
    ...
    <div style="display: flex; align-items: center; margin-top: 12px">
      <el-button type="primary" @click="getData" class="y-btn-icon">
        获取数据
      </el-button>
    </div>
    <file-picker
...
<script lang="ts" setup>
  ...
  const getData = () => {
    ...
    YMessage.success({ message: '数据已打印在控制台', plain: true });
  };
  ...
  const checkFile = (file?: File) => {
    ...
    if (!file.type.startsWith('image')) {
      YMessage.error({ message: '只能选择图片', plain: true });
      return;
    }
    if (file.size / 1024 / 1024 > 2) {
      YMessage.error({ message: '大小不能超过 2MB', plain: true });
      return;
    }
    return true;
  };
  ...
  const handleUpload = (data: UploadItem, retry?: boolean) => {
    ...
    const timer = setInterval(() => {
      if (Math.round(Math.random() * 10) < 3) {
        ...
        YMessage.error({ message: '上传失败, 服务器繁忙', plain: true });
        clearInterval(timer);
        return;
      }
      if (item.progress == null) {
