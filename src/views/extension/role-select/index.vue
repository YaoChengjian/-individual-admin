  ...
  <y-page>
    ...
    <y-card header="自定义数据">
      <div style="margin: 0 0 18px 0">
        角色选择组件基于表格选择组件开发，支持 YTableSelect
...
<script lang="ts" setup>
  ...
  /** 角色选择器选择完成事件 */
  const handleSelect = (result: Role) => {
    selected.value = result;
  };

  /** 角色选择器确认按钮点击事件 */
  const beforeConfirm = (result?: Role) => {
    if (!result) {
      YMessage.error({ message: '请选择一个角色', plain: true });
      return false;
    }
  };

  /** 角色选择器2选中的数据 */
  const selected2 = ref<Role[]>();
  ...
  /** 角色选择器2选择完成事件 */
  const handleSelect2 = (result: Role[]) => {
    selected2.value = result;
  };

  /** 角色选择器2确认按钮点击事件 */
  const beforeConfirm2 = (result?: Role[]) => {
    if (!result?.length) {
      YMessage.error({ message: '请至少选择一个角色', plain: true });
      return false;
    }
  };
  ...
  /** 角色选择器3选中的数据 */
  const selected3 = ref<FileRecord[]>();
  ...
  /** 角色选择器3选择完成事件 */
  const handleSelect3 = (result: FileRecord[]) => {
  ...
  /** 角色选择器4选择完成事件 */
  const handleSelect4 = (result: Role[]) => {
