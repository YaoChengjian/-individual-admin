<template>
  <y-admin-layout
    ...
    :style="{
      background: 'none',
      '--y-header-tool-padding': mobile ? '8px' : '10px'
    }"
  >
    <template #logo>
    ...
    <div :style="{ flex: 'auto' }">
      <el-carousel
        height="320px"
        style="margin-top: calc(0px - var(--y-header-height) - 1px)"
      >
      ...
      >
        <div style="text-align: center; padding: 28px 0 20px 0">
          <y-text size="lg" type="heading">标题标题标题</y-text>
          <y-text size="sm" type="secondary">描述描述描述</y-text>
        </div>
        <el-row :gutter="16">
          <el-col v-for="index in 8" :key="index" :md="6" :sm="8" :xs="12">
            <y-card style="border-radius: 0; min-height: 140px" />
          </el-col>
        </el-row>
      </div>
      <div
      ...
      return [
        {
          ...
          children: menus,
          style: {
            '--y-menu-arrow-size': props.mobile ? '0px' : '14px',
            '--y-menu-horizontal-active-bg': 'transparent',
            '--y-menu-horizontal-line-size': 0,
