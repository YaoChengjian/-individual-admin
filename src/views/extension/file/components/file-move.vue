<template>
  <y-modal :width="460" title="移动到" v-model="visible" @open="handleOpen">
...
<script lang="ts" setup>
  import { ref } from 'vue';
  import { YMessage, toTree, mapTree } from 'y-element-ultra';
  ...
  const save = () => {
    ...
    if (parentId == null) {
      YMessage.error({ message: '请选择分组', plain: true });
      return;
    }
    loading.value = true;
...
<style lang="scss" scoped>
  .demo-file-move-tree {
    ...
    :deep(.el-tree) {
      padding-top: 8px;
      padding-bottom: 8px;
      padding-left: calc(var(--y-tree-item-radius) * 2);
      padding-right: calc(var(--y-tree-item-radius) * 2);
      box-sizing: border-box;
      overflow-x: hidden;
      overflow-y: auto;
      user-select: none;
      --y-tree-item-height: 36px;
      ...
      .el-tree-node__content {
        ...
        & > .el-tree-node__label.is-active {
          ...
          &::before {
            ...
            border-radius: var(--y-tree-item-radius);
          }
        }
