    ...
    >
      <template #toolbar>
        <el-space :size="12" wrap>
          <el-button type="primary" class="y-btn-icon" @click="openExport">
            打开导出弹窗
          </el-button>
          <el-button type="primary" class="y-btn-icon" @click="openPrint">
            打开打印弹窗
          </el-button>
          <el-button type="primary" class="y-btn-icon" @click="exportData">
            直接导出
          </el-button>
          <el-button type="primary" class="y-btn-icon" @click="printData">
            直接打印
          </el-button>
        </el-space>
      </template>
...
<script lang="ts" setup>
  ...
  import { YMessage, random } from 'y-element-ultra';
  ...
  const openEdit = (row: Piece) => {
    YMessage.success({ message: `编辑: ${row.pieceNo}`, plain: true });
  };

  /** 删除单个 */
  const remove = (row: Piece) => {
    YMessage.error({ message: `删除: ${row.pieceNo}`, plain: true });
  };
  ...
  /* const getCurrent = () => {
    if (!current.value) {
      YMessage.success({ message: '未选中任何数据', plain: true });
    } else {
