<template>
  <div>
    ...
    >
      <template #toolbar>
        <el-button type="primary" class="y-btn-icon" @click="getSelections">
          获取多选行数据
        </el-button>
        <el-button
...
<script lang="ts" setup>
  ...
  const openEdit = (row: User) => {
    YMessage.success({ message: `编辑: ${row.nickname}`, plain: true });
  };

  /** 删除单个 */
  const remove = (row: User) => {
    YMessage.error({ message: `删除: ${row.nickname}`, plain: true });
  };
