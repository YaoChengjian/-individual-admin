<template>
  <y-page class="workplace-page">
    ...
    <y-card :body-style="{ padding: 0 }" style="overflow: hidden">
    ...
    <!-- 添加卡片弹窗 -->
    <y-modal :width="680" v-model="visible" title="未添加的卡片">
...
<script lang="ts" setup>
  ...
  /** 卡片数据 */
  const data = ref<ViewItem[]>(getCacheData());
  ...
  const reset = () => {
    ...
    YMessage.success({ message: '已重置', plain: true });
  };
  ...
  const handleCommand = (command: Command, index: number) => {
    switch (command) {
      case 'refresh': // 刷新
        YMessage.info({ message: '点击了刷新', plain: true });
        break;
      case 'edit': // 编辑
        YMessage.info({ message: '点击了编辑', plain: true });
        break;
      case 'remove': // 删除
  ...
  const addView = (item: ViewItem) => {
    ...
    cacheData();
    YMessage.success({ message: '已添加', plain: true });
  };

  onMounted(() => {
    // 卡片支持拖动排序
    sortableIns = new SortableJs(wrapRef.value?.$el, {
      handle: '.y-card-header',
