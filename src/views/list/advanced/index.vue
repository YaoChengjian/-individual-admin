<template>
  <y-page>
      ...
      <el-row>
        <el-col :sm="8" :xs="24">
          <div style="text-align: center">
            <y-text type="placeholder">进行中的任务</y-text>
            <y-text size="xl" style="margin-top: 6px">10 个任务</y-text>
          </div>
        </el-col>
        <el-col :sm="8" :xs="24">
          <div style="text-align: center">
            <y-text type="placeholder">剩余任务</y-text>
            <y-text size="xl" style="margin-top: 6px">3 个任务</y-text>
          </div>
        </el-col>
        <el-col :sm="8" :xs="24">
          <div style="text-align: center">
            <y-text type="placeholder">任务总耗时</y-text>
            <y-text size="xl" style="margin-top: 6px">120 个小时</y-text>
          </div>
        </el-col>
      </el-row>
    </y-card>
    <y-card>
      <!-- 头部工具栏 -->
      <y-toolbar title="复杂列表" :title-props="{ size: 'md' }">
        <template #tools>
      ...
      <div style="margin-top: 8px">
        <el-button :icon="PlusOutlined" class="y-fluid" @click="openEdit()">
          ...
          <div class="list-item">
            <div class="list-item-avatar">
              ...
              <div class="list-item-avatar-extra">
                ...
                <y-text size="sm" type="placeholder">
                  {{ item.content }}
                </y-text>
              </div>
            </div>
            <div class="list-item-owner">
              ...
              <y-text size="sm" type="placeholder">{{ item.user }}</y-text>
            </div>
            <div class="list-item-time">
              <div class="list-item-title">开始时间</div>
              <y-text size="sm" type="placeholder">{{ item.time }}</y-text>
            </div>
  ...
  const handleSubmit = () => {
    formRef.value?.validate?.((valid) => {
      ...
      setTimeout(() => {
        ...
        YMessage.success({ message: '保存成功', plain: true });
        if (form.id) {
          // 保存修改
  ...
  const dropClick = (key: string, item: ListItem) => {
    if (key === 'remove') {
      ...
      ElMessageBox.confirm(`确定删除“${item.title}”吗?`, '系统提示', {
        ...
        .then(() => {
          YMessage.success({ message: '点击了删除', plain: true });
        })
        .catch(() => {});
    } else if (key === 'share') {
      YMessage.success({ message: '点击了分享', plain: true });
    }
  };
