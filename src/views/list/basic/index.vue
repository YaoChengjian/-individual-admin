  ...
  <y-page :flex-table="fixedHeight">
    ...
    >
      ...
      >
        <template #title>
          <span>已选择&nbsp;</span>
          <y-text strong type="primary" tag="span">
            {{ selections.length }}
          </y-text>
          <span>&nbsp;项数据&emsp;</span>
      ...
      >
        <template #toolbar>
          ...
          >
            <el-button :disabled="!selections.length" class="y-btn-icon">
              <span>批量操作</span>
        ...
        <!-- 状态列 -->
        <template #status="{ row }">
          <y-dot v-if="row.status === 0" text="正常" size="8px" />
          <y-dot
...
<script lang="ts" setup>
  ...
  import { getExportWorkbook } from 'y-element-ultra/es/y-pro-table/exceljs-plugin';
  ...
  const handleDropClick = (command: string) => {
    if (command === 'del') {
      YMessage.error({ message: '点击了批量删除', plain: true });
    } else if (command === 'edit') {
      YMessage.info({ message: '点击了批量修改', plain: true });
    } else if (command === 'disabled') {
      YMessage.info({ message: '点击了批量禁用', plain: true });
    }
  };
