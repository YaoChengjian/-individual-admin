...
export async function listUsers(params?: UserParam) {
  const res = await request.post<ApiResult<User[]>>(
    '/system/user/list',
    params
  );
...
export async function getUser(id: number) {
  const res = await request.post<ApiResult<User>>(`/system/user/detail/${id}`);
  if (res.data.code === 0 && res.data.data) {
    return res.data.data;
  }
  return Promise.reject(new Error(res.data.message));
...
export async function addUser(data: User) {
  const res = await request.post<ApiResult<unknown>>('/system/user/add', data);
  if (res.data.code === 0) {
